...
export default class ShipperStatus extends Component {
    ...
    showHistory() {
        const array = []
        const data = this.props.data.trackingInfo.history
        ...
        data.map((item, index) => {
            if (item.localDate !== lastItem.localDate) {
                array.push(
                    ...
                    <TimeLineLabel key={index + item.localDate + item.state + i} value={item.localDate} />
                )
            }
            array.push(

                <TimeLine key={index + item.localDateTime + i}
                    title={<>
    ...
    showStatus() {
        ...
        const data = this.props.data.trackingInfo.current
        let i = 81000
        data.map((item, index) => {
            i++
            array.push(
